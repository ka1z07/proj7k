...
- [ ] `LazerLogWatcher` 能在后台平稳 tail 最新 `runtime.log`，遇到日志轮转时自适应切换；
- [ ] 准确识别提取日志中的 Artist, Title, DifficultyName 与 Creator；
- [ ] 内存索引能在启动时在 0.5s 内构建完成，并在清洗注入后缀（如 `(10.74★ Tech)`）后实现 O(1) 字典寻址；
- [ ] 无法精确匹配时，模糊匹配能有效回退，并支持文件不存在时的优雅错误通知；
- [ ] 编写端到端日志事件注入模拟测试，验证选歌触发完整的 WebSocket 广播。

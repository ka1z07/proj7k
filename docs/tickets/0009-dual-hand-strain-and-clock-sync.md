...
- **解决 Commit**: [`16e6d7c`](https://github.com/ka1z07/proj7k/commit/16e6d7c), [`fa9a6ef`](https://github.com/ka1z07/proj7k/commit/fa9a6ef)
...
- [x] WebSocket 谱面数据帧中完整包含左手与右手连续时序应变数据及 P90、Top5% 阈值；
- [x] 日志监听器能正确捕获时钟点火（Started）与跳转（Seeking）事件，并向客户端推送 `clock_sync` 消息；
- [x] 在谱面未处于游玩状态时，正确标记时钟为空闲/未激活；
- [x] 编写时序组装与时钟对齐事件流的自动化测试。

...
## 修订 1：K_base 不再含 speed（2026-09-22，工单 #47）

**修订内容。** 决策 4 的载体改为

$$K_{\text{base}} = \max(r_{\text{stream\_raw}}, \min(r_{\text{jack\_raw,eff}}, r_{\text{stream\_raw}} \cdot c_{\text{cap}}))$$

即从 max 中**移除 $r_{\text{speed\_raw}}$**。$\Omega_{\text{irreg}}$、耦合式与饱和形式均不变。

**理由。** 原式把每一张 speed 谱都变成了一张「以 speed 为特征的 tech 谱」：tech 是 `K_base × 倍率`，而在爆发速度本身就是难点所在的谱面上 `K_base` 恰好**就是** `r_speed`，于是技巧负荷恒为 `speed × (1.07 ~ 1.15)`。实测 120 谱真值阶梯：Regular Speed 组的 15 张谱有 6 张的最高维是 tech，领先幅度仅 7%–15%；而该组谱面的微爆发率/流速比为 0.608，远高于 Regular Stream 的 0.119——它们是爆发速度谱，玩家群体也如此定级，只是在原定义下被强行标成了 tech。修订后 Regular Speed 组 15/15 张由 speed 主导。

**代价与边界。** tech 的载体只剩流与叠键，因此 tech 要反超载体必须真正依靠排列的无序度：`tech_saturation_gain` 由 0.15 提到 0.8，让「反超基础物理动能」这句话有实际幅度（15% 的封顶落在其它一切量的噪声里）；Regular Tech 组仍有 9/15 张由 tech 主导，其余落在 speed 或 stream 上——按本修订的口径，那些谱面确实是爆发速度或流主导的。

**未修订、但已知的边界。** LN 侧（决策 2）的 `ln_release` 定位维持「作为修饰项、仅在特定谱面超越 General」不变，但读的量由「反相率 + 释放率」改为**抬手时同手仍锁着其他键的深度**（`release_lock_depth`，见工单 #47）：前者数的是**有伴奏**的抬手，与决策 2 想要的「纯跳音释放」恰好相反，且作为加项永远压不过基座（实测与其自身梯队的 General 维相关系数 ρ=0.987，15 张谱 0 张成为最高维）。

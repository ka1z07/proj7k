...
def test_format_injected_difficulty_name():
    assert format_injected_difficulty_name("Hard", 6.42, "jack") == "Hard (6.42★ Jack)"
    assert format_injected_difficulty_name("Hard", 6.42, "Jack") == "Hard (6.42★ Jack)"
    assert format_injected_difficulty_name("Insane", 8.0, "ln_general") == "Insane (8.00★ LN_General)"
    assert format_injected_difficulty_name("Master", 9.156, "LN_Release") == "Master (9.16★ LN_Release)"
    assert format_injected_difficulty_name("", 5.0, "speed") == "(5.00★ Speed)"
...
def test_difficulty_name_idempotence():
    ...
    assert formatted_1 == "Insane (6.42★ Jack)"
    ...
    assert updated == "Insane (7.10★ Tech)"

    # 3. Repeated stripping yields base name
...
def test_generate_binned_skill_tags():
    assert generate_binned_skill_tags("jack", 6.42) == ["dominant_jack", "jack_6★"]
    assert generate_binned_skill_tags("LN_General", 7.8) == ["dominant_ln_general", "ln_general_7★"]
    assert generate_binned_skill_tags("Tech", 5.0) == ["dominant_tech", "tech_5★"]
    assert generate_binned_skill_tags("speed", 0.8) == ["dominant_speed", "speed_0★"]


def test_inject_binned_skill_tags():
    # Empty tags
    assert inject_binned_skill_tags("", "jack", 6.42) == "dominant_jack jack_6★"
    assert inject_binned_skill_tags("   ", "jack", 6.42) == "dominant_jack jack_6★"

    # Existing tags without prior binned tags
    assert (
        inject_binned_skill_tags("rock anime", "jack", 6.42)
        == "rock anime dominant_jack jack_6★"
    )

    # Existing tags already having the exact same binned tags (deduplication)
    assert (
        inject_binned_skill_tags("rock anime dominant_jack jack_6★", "jack", 6.42)
        == "rock anime dominant_jack jack_6★"
    )

    # Update: replacing previous binned tags with new technique and rating
    assert (
        inject_binned_skill_tags("rock anime dominant_tech tech_5★", "jack", 6.42)
        == "rock anime dominant_jack jack_6★"
    )
...
def test_strip_binned_skill_tags():
    assert (
        strip_binned_skill_tags("rock anime dominant_jack jack_6★")
        == "rock anime"
    )
    assert strip_binned_skill_tags("dominant_jack jack_6★") == ""
...
def test_tags_injection_idempotence():
    ...
    assert injected_1 == "electronic instrumental dominant_stream stream_7★"
...
def test_annotate_lazer_beatmap_7k():
    ...
    assert payload.difficulty_name == "Hyper (6.78★ Stream)"
    assert payload.tags == "jubeat dominant_stream stream_6★"
...
def test_create_lazer_revert_payload():
    record = _make_dummy_record(
        difficulty_name="Hyper (6.78★ Stream)",
        tags="jubeat dominant_stream stream_6★",
        star_rating=6.78,
    )
...
def test_annotate_batch_and_build_collections():
    ...
    assert updates[0].difficulty_name == "Easy (3.20★ Jack)"
    assert updates[1].difficulty_name == "Hard (7.50★ LN_Inverse)"
...
def test_space_in_dominant_tech_sanitized_for_tags():
    ...
    assert tags == ["dominant_ln_general", "ln_general_7★"]
    injected = inject_binned_skill_tags("anime", "LN General", 7.2)
    assert injected == "anime dominant_ln_general ln_general_7★"
...
def test_none_dominant_technique_fallback():
    ...
    assert tags == ["dominant_tech", "tech_0★"]
